	...
	class bookingpress_pro_staff_members Extends BookingPress_Core {
		...
		*/
		...
		function bookingpress_retrieve_staff_holidays( $retrieve_default_holidays, $selected_service, $selected_service_duration, $selected_staffmember ){
			...
			$retrieve_daysoff = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$tbl_bookingpress_staffmembers_daysoff} WHERE bookingpress_staffmember_id = %d AND bookingpress_staffmember_daysoff_parent = %d", $selected_staffmember, 0 ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Reason: $tbl_bookingpress_staffmembers_daysoff is table name defined globally. False Positive alarm 
				...
				if ( ! empty( $bookingpress_staffmember_daysoff_details ) ) {
					foreach ( $bookingpress_staffmember_daysoff_details as $day_off ) {
						...
						if( true == $day_off_arr['dayoff_repeat'] ){
							...
							$day_off_arr['dayoff_repeat_label'] = $this->bookingpress_retrieve_daysoff_repeat_label( $repeat_duration, $repeat_frequency, $repeat_frequency_type, $repeat_times, $repeat_date );
						}
			...
			if( is_plugin_active( 'bookingpress-cart/bookingpress-cart.php') ){
				...
				if( !empty( $appointment_data_obj['cart_items'] ) ){
					...
					foreach( $cart_items as $ck => $cart_data ){
						if( $cart_data['bookingpress_store_selected_date'] == $selected_date ){
			...
			if( is_plugin_active( 'bookingpress-location/bookingpress-location.php' ) && !empty( $bookingpress_location_id ) && 'true' == $bpa_staff_on_multiple_locations ){
				...
				if( !empty( $bookingpress_location_staff_default_workhours ) ){
					foreach( $bookingpress_location_staff_default_workhours as $staff_location_working_data ){
						...
						if( in_array( $staff_id, $staff_with_holidays) || in_array( $staff_id, $exclude_location_staff_workhours ) ){
							continue;
						}
		...
		function bookingpress_validate_staffmember_daysoff_func() {
			...
			if( true == $staff_form_data['dayoff_repeat'] ){
				...
				$dayoff_label = $this->bookingpress_retrieve_daysoff_repeat_label( $repeat_duration, $repeat_frequency, $repeat_frequency_type, $repeat_times, $repeat_date );
			}
